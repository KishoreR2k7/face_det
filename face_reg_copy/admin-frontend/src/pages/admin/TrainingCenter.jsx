...
const TrainingCenter = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStudents(data.map(s => ({
          ...
          department: 'N/A',
          year: 'N/A',
          trained: true // Assume trained if in database
        })));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    await withLoading(async () => {
      try {
        ...
        // Step 2: Upload dataset images/videos
        const formData = new FormData();
        files.forEach((file) => {
          formData.append('files', file);
        });

        const uploadResponse = await fetch(`http://localhost:5000/dataset/upload/${values.roll_no}`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
          },
        ...
        toast.success('Student added and dataset uploaded successfully!');
        
        if (uploadData.training_completed) {
          toast.success('Face recognition model trained successfully!');
          setTrainingStatus('completed');
          setTrainingProgress(100);
        } else {
          toast.warning('Training in progress...');
  ...
  const retrainStudent = async (studentId) => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/dataset/train', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
