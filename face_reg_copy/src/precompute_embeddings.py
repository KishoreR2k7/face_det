...
def precompute_embeddings():
    ...
    for person_name in tqdm(person_folders, desc="Generating Embeddings"):
        ...
        # Move this block outside the image loop (dedent one level)
        if person_embeddings:
            ...
            if len(person_embeddings.shape) == 2:  # Valid embeddings array
                ...
                print(f"✅ Added embeddings for {person_name} - {len(person_embeddings)} images processed")
            else:
                print(f"⚠️ Skipping {person_name} - Invalid embeddings shape: {person_embeddings.shape}")
        else:
